...
public class BinaryPacketFactory {
    ...
    public byte[] packetWithPayload(byte[] payload) throws SecureChannelException {
	try {

	    byte[] load = keyInformation.outgoingCompression.compress(payload, null);

	    int length = 5 + load.length;

	    int mod = Math.max(keyInformation.outgoingCipher.cipherBlockSize(), 8);
	    ...
	    byte[] seqNum = ByteArrayUtils.toByteArray(keyInformation.outgoingSeqNumber++);
    ...
    public byte[] payloadOfPacket(byte[] packet, byte[] mac) throws SecureChannelException {
	try {
	    int mod = Math.max(keyInformation.incomingCipher.cipherBlockSize(), 8);

	    if (packet.length % mod != 0) {
		throw new InvalidPacketException("Packet length was smaller than expected!");
	    }
