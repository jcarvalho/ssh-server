...
public class SSHSecureChannel extends SecureChannel {
	...
	@Override
	public String doSetup() throws IOException, SecureChannelException {
		...
		output.write(binaryPacket.packetWithPayload(packet
		...
		while (true) {
			...
			if (input.read(first) == -1) {
				...
				throw new SecureChannelException(
						"SSH Secure Channel Setup failed!");
			}
			...
			if (first.length != numBytesForPacket) {
				input.read(buffer, first.length, numBytesForPacket
						- first.length);
			}
			...
			SSHPacket pack = SSHPacketFactory.processPacket(reconstructed,
					keyInformation);
			...
			if (response != null) {
				...
				if (respBytes != null) {
					byte[] responseRepr = binaryPacket
							.packetWithPayload(respBytes);
					output.write(responseRepr);
					// FIXME Terrible hack!
					if (response instanceof DHGexReply
							|| response instanceof DHKexReply) {
	...
	@Override
	public String readLine() throws IOException, SecureChannelException {
		if (state != ChannelState.EXECUTING)
			throw new ChannelIOException(
					"ERROR: Trying to read a line from a closed connection!");
	...
	@Override
	public void write(String string) throws IOException, SecureChannelException {
		if (state != ChannelState.EXECUTING)
			throw new ChannelIOException(
					"ERROR: Trying to write a line from to closed connection!");

		String str = string + (string.length() == 0 ? "" : "\r\n") + "> ";

		ChannelData packet = new ChannelData(keyInformation.channel,
				str.getBytes("UTF-8"));
	...
	@Override
	public void close(int code) throws IOException, SecureChannelException {
		...
		ChannelRequest packet = new ChannelRequest(keyInformation.channel,
				"exit-status", false, ByteArrayUtils.toByteArray(code));
