...
import pt.jcarvalho.ssh.common.cipher.Cipher3DES;
import pt.jcarvalho.ssh.common.cipher.CipherAES;
...
public class KExInitPacket extends SSHPacket {
	...
	public String print() {
		...
		buff.append("encryption_algorithms_client_to_server: "
				+ names[2].getNames() + "\n");
		buff.append("encryption_algorithms_server_to_client: "
				+ names[3].getNames() + "\n");
		buff.append("mac_algorithms_client_to_server: " + names[4].getNames()
				+ "\n");
		buff.append("mac_algorithms_server_to_client: " + names[5].getNames()
				+ "\n");
		buff.append("compression_algorithms_client_to_server: "
				+ names[6].getNames() + "\n");
		buff.append("compression_algorithms_server_to_client: "
				+ names[7].getNames() + "\n");
	...
	public void initWithNameLists(boolean firstFollows, NameList... lists) {
		...
		int offset = 17;
		for (NameList list : lists) {
			System.arraycopy(list.toByteArray(), 0, res, offset,
					list.getTotalLength());
	...
	public void process() {
		...
		for (String prefIncEnc : names[2].getNames()) {
			if (prefIncEnc.startsWith("aes") && prefIncEnc.endsWith("-cbc")) {
				...
				keyInformation.TincomingCipher = CipherAES.cipherWithSize(size);
				break;
			} else if (prefIncEnc.equals("3des-cbc")) {
				keyInformation.TincomingCipher = new Cipher3DES();
				break;
			}
		}
		...
		for (String prefOutEnc : names[3].getNames()) {
			if (prefOutEnc.startsWith("aes") && prefOutEnc.endsWith("-cbc")) {
				...
				keyInformation.ToutgoingCipher = CipherAES.cipherWithSize(size);
				break;
			} else if (prefOutEnc.equals("3des-cbc")) {
				keyInformation.ToutgoingCipher = new Cipher3DES();
				break;
			}
		}
		...
		// checking
		keyInformation.TincomingCompression = new CompressionNone();
		keyInformation.ToutgoingCompression = new CompressionNone();

		if (keyInformation.kex == null
				|| keyInformation.TincomingCipher == null
				|| keyInformation.ToutgoingCipher == null
				|| keyInformation.TincomingMAC == null
				|| keyInformation.ToutgoingMAC == null) {
			valid = false;
		}
	...
	public SSHPacket nextPacket() {
		return (valid ? null : new Disconnect("Unsupported algorithm!",
